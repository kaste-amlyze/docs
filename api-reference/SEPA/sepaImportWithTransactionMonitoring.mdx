...
<Note>
Further examples and descriptions applies only for SEPA operation import with transaction monitoring.
</Note>
...
Below are examples of API requests for SEPA operation import with transaction monitoring initiation with only mandatory fields and all fields in JSON format, as well as a list of all available fields. 
