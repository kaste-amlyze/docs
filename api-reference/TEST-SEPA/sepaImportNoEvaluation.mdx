...
! Further examples and descriptions applies only for SEPA operation import without evaluation.
...
Below are examples of API requests for SEPA operation import without evaluation initiation with only mandatory fields and all fields in JSON format, as well as a list of all available fields. 
...
<Expandable title="Mandatory fields example | SEPA operation import - no evaluation">
<ResponseField>
  ```json
{
...
<Expandable title="All fields example | SEPA operation import - no evaluation">
<ResponseField>
  ```json
{
