---
title: 'CARD-PAYMENT operation import - with transaction monitoring'
icon: 'arrow-right'
---

<Note>
Just so you know, further examples and descriptions apply only to CARD-PAYMENT operation imports with transaction monitoring.
</Note>
...
<Expandable title="Mandatory fields example | CARD-PAYMENT operation import - with transaction monitoring">
<ResponseField>
  ```json
{
    "communicationNumber": "Com_1455",
    "requester": "financial_institution",

    "operationExtId": "ExtId_879",
...
<Expandable title="Mandatory fields example | CARD-PAYMENT operation import - with transaction monitoring and screening">
<ResponseField>
  ```json
{
    "communicationNumber": "Com_1455",
...
<Expandable title="All fields example | CARD-PAYMENT operation import - with transaction monitoring">
<ResponseField>
  ```json
{
...
_Example:_ `Op_01_CARD_PAYMENT_TEST`
</ResponseField>

<ResponseField name="operationType" type="string" required>
Notifies about which kind of operation type was performed.

_Value:_ `CARD_PAYMENT`
</ResponseField>

<ResponseField name="cardOperationSubType" type="string from classifier list" required>
Card operation subtype code. These field values are configurable individually.

Common examples: [CARD_PURCHASE,
CARD_ACCOUNT_FUNDING_TRANSACTION,
CARD_PAYMENT_CREDIT
CARD_REFUND]

_Example:_ `CARD_PURCHASE`

<Expandable title="Path to extract classifier values">
**HTTP Method:**  `GET`

**Path:** `{{baseUrl}}`/classifier/cardOperationSubType
</Expandable>
...
Direction `INCOMING`, your customer is `CREDITOR`,
direction `OUTGOING`, your customer is `DEBTOR`.
