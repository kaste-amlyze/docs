...
<Note>
Just so you know, further examples and descriptions apply only to PIS operation imports without transaction monitoring.
</Note>
...
<Expandable title="Mandatory fields example | PIS operation import - no transaction monitoring">
<ResponseField>
  ```json
{
...
<Expandable title="Mandatory fields example | PIS operation import - no transaction monitoring with screening">
<ResponseField>
  ```json
{
    "communicationNumber": "Test_ComNr_PIS_05",
...
<Expandable title="All fields example | PIS operation import - no transaction monitoring">
<ResponseField>
  ```json
{
...
**Enum class values:** `CREDITOR`, `DEBTOR`
